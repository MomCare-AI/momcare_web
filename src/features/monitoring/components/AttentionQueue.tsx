...
interface Props {
  /** The dashboard shows a short list; the full queue has its own page later. */
  limit?: number;
}
 ...
 */
export function AttentionQueue({ limit }: Props) {
  ...
  if (rows.length === 0) {
    return (
      <div className="mc-empty">
        ...
        <span className="mc-empty-text">
          No monitored patient is currently outside range. Patients appear here
          the moment a reading crosses a clinical threshold — or stops arriving.
        </span>
      </div>
    );
  }
