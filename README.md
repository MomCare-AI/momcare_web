...
A Next.js frontend for a maternal healthcare platform: a marketing site plus a
single authenticated hospital portal at `/dashboard`, whose views adapt to the
signed-in user's role (hospital admin, provider, nurse, or care manager). It's
a pure client for a separate **Django REST API** backend; there is no database
or server-owned data layer in this repo.
...
| HTTP client                  | Native `fetch`, wrapped in `core/api/authFetch.ts` (token attach + single-retry refresh on 401)                                                              |
...
- `src/app/` — routing only, via Next.js App Router. `(auth)` (login,
  register, invite, password reset) and `(portal)` (the authenticated
  hospital dashboard at `/dashboard`) are route groups; the marketing
  page lives at the root.
...
src/
  ...
  features/     business domains — patients, monitoring, alerts, staff,
                portal, hospital-onboarding, dashboard
...
public/         static assets + locales (en, ur — not yet wired up)
```
